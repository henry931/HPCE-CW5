...
void process_recursive_function_sse_8(unsigned recursionlevel,const unsigned w,const unsigned h,const int inputhandle,const int outputhandle, std::vector<std::vector<__m128i>> &pixelsA, std::vector<std::vector<__m128i>> &pixelsB, std::vector<std::vector<__m128i>> &pixelsC, std::vector<uint32_t> &line, std::vector<__m128i> &output, std::vector<uint32_t> &status, const uint32_t toplevel){
	...
	if (*thisline == 0) {
		...
		if (recursionlevel == 0){
			...
			process_recursive_function_sse_8(recursionlevel-1, w, h, inputhandle, outputhandle, pixelsA, pixelsB, pixelsC, line, *pixelptr_vec[0], status, toplevel);
		}
	...
	if (*thisline == 1) {
		...
		if (recursionlevel == 0){
			...
			process_recursive_function_sse_8(recursionlevel-1, w, h, inputhandle, outputhandle, pixelsA, pixelsB, pixelsC, line, *pixelptr_vec[1], status, toplevel);
		}

		// Process first line
		if( status[recursionlevel] == 1){
			dilate_line_top_sse_8(w, *pixelptr_vec[0], *pixelptr_vec[1], output);
			//for(unsigned i = 0; i < w/16; i++) output[i] = pixelptr[0][i];
		}
		else if( status[recursionlevel] == 0){
	...
	while(*thisline < h) {
		...
		if (recursionlevel == 0){
			...
			process_recursive_function_sse_8(recursionlevel-1, w, h, inputhandle, outputhandle, pixelsA, pixelsB, pixelsC, line, *pixelptr_vec[*thisline%3], status, toplevel);
		}

		if( status[recursionlevel] == 1 ){
			dilate_line_sse_8(w, *pixelptr_vec[(*thisline+1)%3], *pixelptr_vec[(*thisline+2)%3], *pixelptr_vec[*thisline%3], output);
			//for(unsigned i = 0; i < w/16; i++) output[i] = pixelptr[(*thisline+2)%3][i];
		}
		else if( status[recursionlevel] == 0 ){
...
void process_recursive_sse_8(const int levels, const unsigned w, const unsigned h,const int inputhandle,const int outputhandle){
	...
	process_recursive_function_sse_8(reclevel, w, h, inputhandle, outputhandle, pixelsA, pixelsB, pixelsC, line, outbuff, status, reclevel);
